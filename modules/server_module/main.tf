...
resource "null_resource" "copy-file-from-master" {
  count             = var.is_master ? 1 : 0
    ...
    command = "scp -i /home/agent/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ubuntu@${aws_instance.server.public_ip}:/home/ubuntu/kubeadm_join.txt ${var.local_kubeadm_path}"
  }
